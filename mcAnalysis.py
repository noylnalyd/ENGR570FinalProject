...
for i in range(nParam):
    ...
    for j in range(nSamples):
        num=num+(fA.iloc[j]["Time to Stable Temp"]*fBA.iloc[j]["Time to Stable Temp"]-fA.iloc[j]["Time to Stable Temp"]*fB.iloc[j]["Time to Stable Temp"])
        numT=numT+(fA.iloc[j]["Time to Stable Temp"]-fAB.iloc[j]["Time to Stable Temp"])**2
    for j in range(2*nSamples):
        denom=denom+(fC.iloc[j]["Time to Stable Temp"]**2)
    num=num/nSamples
